...
ESX.RegisterServerCallback('SmallTattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('esx:showNotification', source, "Du købte ~y~" .. tattooName .. "~s~  for ~g~" .. price)
		cb(true)
	else
		TriggerClientEvent('esx:showNotification', source, "Du har ikke penge nok")
		cb(false)
	end
end)
