...
function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "For at blive tattoveret brug", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Skift kamara", 21, "Ændre gennemsigtighed", {90, 89}, "Køb/Fjern Tattoo", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	JayMenu.CreateMenu("tattoo", "Devil\'s inc", function()
        return CloseTattooShop()
    end)
    JayMenu.SetSubTitle('tattoo', "Kategorier")
