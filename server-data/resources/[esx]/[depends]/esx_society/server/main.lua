	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('have_withdrawn') })
				--xPlayer.showNotification(_U('have_withdrawn', ESX.Math.GroupDigits(amount)))
			else
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('invalid_amount') })
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('have_deposited') })
...
AddEventHandler('esx_society:washMoney', function(society, amount)
	...
	if xPlayer.job.name == society then
		if amount and amount > 0 and account.money >= amount then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('you_have') })
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('you_have_been_hired') })
				--xTarget.showNotification(_U('you_have_been_hired', job))
			elseif type == 'promote' then
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('you_have_been_promoted') })
				--xTarget.showNotification(_U('you_have_been_promoted'))
			elseif type == 'fire' then
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('you_have_been_fired') })
...
function WashMoneyCRON(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('you_have_laundered') })
