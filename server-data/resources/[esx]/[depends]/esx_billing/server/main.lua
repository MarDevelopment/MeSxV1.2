		...
		TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
			if account then
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)', {
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('received_invoice') })
			...
			if result[1].target_type == 'player' then
				if xTarget then
					if xPlayer.getMoney() >= amount then
						MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
							...
							if rowsChanged == 1 then
								...
								TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('paid_invoice') })
								--xPlayer.showNotification(_U('paid_invoice', ESX.Math.GroupDigits(amount)))
								TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('received_payment') })
					...
					elseif xPlayer.getAccount('bank').money >= amount then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('target_no_money') })
						--xTarget.showNotification(_U('target_no_money'))
						TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('no_money') })
