	...
	if xPlayer and xPlayer.job.name == 'ambulance' then
		...
		if xTarget then
			if deadPlayers[playerId] then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('revive_complete_award', xTarget.name, Config.ReviveReward) })
					--xPlayer.showNotification(_U('revive_complete_award', xTarget.name, Config.ReviveReward))
					xPlayer.addMoney(Config.ReviveReward)
					xTarget.triggerEvent('esx_ambulancejob:revive')
					deadPlayers[playerId] = nil
				else
				TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('revive_complete', xTarget.name) })
					--xPlayer.showNotification(_U('revive_complete', xTarget.name))
					xTarget.triggerEvent('esx_ambulancejob:revive')
					deadPlayers[playerId] = nil
				end
			else
			TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('player_not_unconscious') })
				--xPlayer.showNotification(_U('player_not_unconscious'))
			end
		else
		TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('revive_fail_offline') })
...
if Config.EarlyRespawnFine then
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('respawn_bleedout_fine_msg', ESX.Math.GroupDigits(fineAmount)) })
...
AddEventHandler('esx_ambulancejob:removeItem', function(item)
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('used_bandage') })
		--xPlayer.showNotification(_U('used_bandage'))
	elseif item == 'medikit' then
	TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('used_medikit') })
...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName, amount)
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('max_item') })
