...
ESX.RegisterServerCallback('esx_advancedhospital:payHealing', function(source, cb)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('healing_paid', ESX.Math.GroupDigits(price)) })
...
ESX.RegisterServerCallback('esx_advancedhospital:paySurgery', function(source, cb)
	...
	if xPlayer.getMoney() >= price then
		xPlayer.removeMoney(price)
		TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('surgery_paid', ESX.Math.GroupDigits(price)) })
