...
function SetPropertyOwned(name, price, rented, owner)
	MySQL.Async.execute('INSERT INTO owned_properties (name, price, rented, owner) VALUES (@name, @price, @rented, @owner)', {
		...
		if xPlayer then
			...
			if rented then
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('rent_for', ESX.Math.GroupDigits(price)) })
				--xPlayer.showNotification(_U('rent_for', ESX.Math.GroupDigits(price)))
			else
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('buy_for', ESX.Math.GroupDigits(price)) })
			...
			MySQL.Async.execute('DELETE FROM owned_properties WHERE id = @id', {
				...
				if xPlayer then
					...
					if not noPay then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('moved_out') })
							--xPlayer.showNotification(_U('moved_out'))
						else
							local sellPrice = ESX.Math.Round(result[1].price / Config.SellModifier)
						TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('moved_out_sold', ESX.Math.GroupDigits(sellPrice)) })
...
AddEventHandler('esx_property:buyProperty', function(propertyName)
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('not_enough') })
...
AddEventHandler('esx_property:getItem', function(owner, type, item, count)
		...
		TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if xPlayer.canCarryItem(item, count) then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('have_withdrawn', count, inventoryItem.label) })
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('not_enough_in_property') })
	...
	elseif type == 'item_account' then
		TriggerEvent('esx_addonaccount:getAccount', 'property_' .. item, xPlayerOwner.identifier, function(account)
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('amount_invalid') })
	...
	if type == 'item_standard' then
		...
		if playerItemCount >= count and count > 0 then
			TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('have_deposited', count, inventory.getItem(item).label) })
		...
		for k,v in ipairs(result) do
			table.insert(tasks, function(cb)
				...
				if xPlayer then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('paid_rent', ESX.Math.GroupDigits(v.price), GetProperty(v.name).label) })
						--xPlayer.showNotification(_U('paid_rent', ESX.Math.GroupDigits(v.price), GetProperty(v.name).label))
					else
					TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('paid_rent_evicted', GetProperty(v.name).label, ESX.Math.GroupDigits(v.price)) })
