...
AddEventHandler('esx_holdup:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer[i], { type = 'inform', text = _U('robbery_cancelled_at', Stores[currentStore].nameOfStore) })
	...
	if robbers[_source] then
		...
		robbers[_source] = nil
		TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'inform', text = _U('robbery_cancelled_at', Stores[currentStore].nameOfStore) })
					...
					if robbers[_source] then
						...
						if xPlayer then
							...
							for i=1, #xPlayers, 1 do
								...
								if xPlayer.job.name == 'police' then
								TriggerClientEvent('mythic_notify:client:SendAlert', xPlayers[i], { type = 'inform', text = _U('robbery_complete_at', store.nameOfStore) })
