...
AddEventHandler('esx_drugs:sellDrug', function(itemName, amount)
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('dealer_notenough') })
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', amount, { type = 'inform', text = _U('dealer_sold', amount, xItem.label, ESX.Math.GroupDigits(price)) })
...
AddEventHandler('esx_drugs:pickedUpCannabis', function()
	...
	if xPlayer.canCarryItem('cannabis', cime) then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('weed_inventoryfull') })
      ...
      while outofbound == false and can do
            ...
            if xCannabis.count >= 3 then
              if xPlayer.canSwapItem('cannabis', 3, 'marijuana', 1) then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('weed_processed') })
								--xPlayer.showNotification(_U('weed_processed'))
							else
								can = false
								TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('weed_processingfull') })
								--xPlayer.showNotification(_U('weed_processingfull'))
								TriggerEvent('esx_drugs:cancelProcessing')
							end
						else						
							can = false
							TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('weed_processingenough') })
