...
AddEventHandler('esx_advancedjail:rununjail', function(_source)
	...
	if xPlayer then
		MySQL.Async.execute('UPDATE users SET jail_time = @jail_time, jail_loc = @jail_loc WHERE identifier = @identifier', {
			...
			playersInJail[playerId] = nil
			xPlayer.triggerEvent('esx_advancedjail:unjailPlayer')
			TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('you_ran') })
...
AddEventHandler('esx_advancedjail:playerEscape', function(name, type, location)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			if type == 'run' then
				if location == 'mr' then
				TriggerClientEvent('mythic_notify:client:SendAlert',{ type = 'inform', text = _U('player_escaped', name, _U('mission_row')) })
					--xPlayer.showNotification(_U('player_escaped', name, _U('mission_row')))
				elseif location == 'bp' then
				TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('player_escaped', name, _U('bolingbroke_pen')) })
					--xPlayer.showNotification(_U('player_escaped', name, _U('bolingbroke_pen')))
				end
			elseif type == 'attempt' then
				if location == 'mr' then
				TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('player_escape_attempt', name, _U('mission_row')) })
					--xPlayer.showNotification(_U('player_escape_attempt', name, _U('mission_row')))
				elseif location == 'bp' then
				TriggerClientEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('player_escape_attempt', name, _U('bolingbroke_pen')) })
