...
AddEventHandler('esx_extraitems:bandage', function(source)
	...
	exports['mythic_notify']:SendAlert('Inform', _U('error_veh'))
		...
		exports['mythic_notify']:SendAlert('Inform', _U('error_no_foot'))
...
AddEventHandler('esx_extraitems:defib', function(source)
	...
	exports['mythic_notify']:SendAlert('Inform', _U('error_veh'))
		...
		if IsPedOnFoot(playerPed) then
			...
			exports['mythic_notify']:SendAlert('Inform', _U('error_no_ped'))
				...
				if IsPedDeadOrDying(closestPlayerPed, 1) then
					...
					if chance <= 66 then
						...
						exports['mythic_notify']:SendAlert('Inform', _U('defib_worked'))
						--ESX.ShowNotification(_U('defib_worked'))
					else
					exports['mythic_notify']:SendAlert('Inform', _U('defib_failed'))
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				...
				if chance <= 66 then
					...
					ClearPedTasksImmediately(playerPed)
					exports['mythic_notify']:SendAlert('Inform', _U('veh_unlocked'))
					--ESX.ShowNotification(_U('veh_unlocked'))
				else
					TriggerServerEvent('esx_extraitems:removelockpick')
					exports['mythic_notify']:SendAlert('Inform', _U('hijack_failed'))
...
exports['mythic_notify']:SendAlert('Inform', _U('dive_suit_on') .. '%.')
			--ESX.ShowNotification(_U('dive_suit_on') .. '%.')

			Citizen.Wait(50000)
			exports['mythic_notify']:SendAlert('Inform', _U('oxygen_notify', '~y~', '50') .. '%.')
			--ESX.ShowNotification(_U('oxygen_notify', '~y~', '50') .. '%.')

			Citizen.Wait(25000)
			exports['mythic_notify']:SendAlert('Inform', _U('oxygen_notify', '~o~', '25') .. '%.')
			--ESX.ShowNotification(_U('oxygen_notify', '~o~', '25') .. '%.')

			Citizen.Wait(25000)
			exports['mythic_notify']:SendAlert('Inform', _U('oxygen_notify', '~r~', '0') .. '%.')
		...
		if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
			if DoesEntityExist(vehicle) and IsPedOnFoot(playerPed) and closestTire ~= nil then
				...
				Citizen.CreateThread(function()
					...
					exports['mythic_notify']:SendAlert('Inform', _U('tire_done'))
					--ESX.ShowNotification(_U('tire_done'))
				end)
			else
			exports['mythic_notify']:SendAlert('Inform', _U('error_no_tire'))
				--ESX.ShowNotification(_U('error_no_tire'))
			end
		else
		exports['mythic_notify']:SendAlert('Inform', _U('error_no_veh'))
			--ESX.ShowNotification(_U('error_no_veh'))
		end
	end
end)
...
AddEventHandler('esx_extraitems:installGPS', function()
	if Config.UseVehicleGPS then
		...
		if ShowRadar == false then
			if DoesEntityExist(playerVeh) then
				if Config.LimitedVehicles then
					...
					if IsThisModelABoat(model) or IsThisModelACar(model) or IsThisModelAHeli(model) or IsThisModelAPlane(model) or IsThisModelAnAmphibiousCar(model) then
						ShowRadar = true
						exports['mythic_notify']:SendAlert('Inform', _U('gps_installed'))
						--ESX.ShowNotification(_U('gps_installed'))
					else
					exports['mythic_notify']:SendAlert('Inform', _U('gps_not_correct'))
			...
			exports['mythic_notify']:SendAlert('Inform', _U('gps_no_vehicle'))
				--ESX.ShowNotification(_U('gps_no_vehicle'))
			end
		else
			DisplayRadar(false)
			ShowRadar = false
			exports['mythic_notify']:SendAlert('Inform', _U('gps_removed'))
...
AddEventHandler('esx_extraitems:checkammo', function(type)
	...
	if IsPedArmed(playerPed, 4) then
		if type == 'boxpistol' then
			...
			exports['mythic_notify']:SendAlert('Inform', _U('not_suitable'))
	...
	exports['mythic_notify']:SendAlert('Inform', _U('no_weapon'))
