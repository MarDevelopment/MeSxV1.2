	...
	if xPlayer.job.name == 'taxi' then
		if not lastPlayerSuccess[source] or timeNow - lastPlayerSuccess[source] > 5 then
			...
			TriggerEvent('esx_addonaccount:getSharedAccount', 'society_taxi', function(account)
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('comp_earned', societyMoney, playerMoney) })
					--xPlayer.showNotification(_U('comp_earned', societyMoney, playerMoney))
				else
					xPlayer.addMoney(total)
					TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('have_earned', total) })
