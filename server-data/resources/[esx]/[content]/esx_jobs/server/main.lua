			...
			if xPlayer then
				...
				if distance <= data.zoneMaxDistance then
					...
					if timeElapsed > data.jobItem[1].time then
						...
						TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('max_limit', v.name) })
							...
							TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('not_enough', data.jobItem[1].requires_name) })
		...
		if cautionAmount <= Config.MaxCaution and cautionAmount > 0 then
			TriggerEvent('esx_addonaccount:getAccount', 'caution', xPlayer.identifier, function(account)
				if xPlayer.getAccount('bank').money >= cautionAmount then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('bank_deposit_taken', ESX.Math.GroupDigits(cautionAmount)) })
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('caution_afford', ESX.Math.GroupDigits(cautionAmount)) })
	...
	elseif cautionType == 'give_back' then
		if cautionAmount <= 1 and cautionAmount > 0 then
			TriggerEvent('esx_addonaccount:getAccount', 'caution', xPlayer.identifier, function(account)
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = _U('bank_deposit_returned', ESX.Math.GroupDigits(toGive)) })
